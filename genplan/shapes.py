...
def concert(w, d):
    ...
    parts = [
        ...
        CAN(canopy, 6.4, 6.9, 'flat', (), z0=0.0, over=0.9),
...
def playground(w, d):
    """Детская площадка по референсу: две круглые башни-домики на деревянных
    опорах с верёвочным мостом, трубчатая горка, скалодром, качели с гнездом,
    круглый павильон на колоннах, домик с горкой, песочница с бортом,
    стела «Детская площадка» и столбики освещения."""
    base = _stadium(w, d, r=d * 0.44)
    t1 = Point(-w * 0.33, d * 0.10).buffer(3.4, 24)      # большая башня
    t2 = Point(-w * 0.06, d * 0.06).buffer(2.8, 24)      # малая башня
    pav = Point(w * 0.30, d * 0.02).buffer(5.0, 32)      # павильон
    house = _box(4.0, 3.4, w * 0.44, -d * 0.24)          # домик с горкой
    sand = Point(w * 0.05, -d * 0.28).buffer(3.2, 24)    # песочница
    return [
        ('platform', dict(poly=base, h=0.12)),
        # башня 1: опоры, объём, ограждение-решётка, кровля
        ('column', dict(pts=[(-w * 0.33 + 2.2 * math.cos(a), d * 0.10 + 2.2 * math.sin(a),
                              0.26) for a in [0.8, 2.4, 4.0, 5.6]], h=4.2)),
        V(t1, 6.6, 7.2, 'flat', z0=4.2, over=0.5),
        RAIL([(p[0], p[1]) for p in cols_ring(t1, 13, 0.1, 0.35)] , 1.1),
        # башня 2
        ('column', dict(pts=[(-w * 0.06 + 1.8 * math.cos(a), d * 0.06 + 1.8 * math.sin(a),
                              0.24) for a in [0.8, 2.4, 4.0, 5.6]], h=3.4)),
        V(t2, 5.6, 6.1, 'cone', z0=3.4, over=0.6),
        RAIL([(p[0], p[1]) for p in cols_ring(t2, 11, 0.1, 0.3)], 1.0),
        # верёвочный мост между башнями
        ('deck', dict(poly=_box(w * 0.26, 1.4, -w * 0.195, d * 0.08), h=0.12)),
        RAIL([(-w * 0.31, d * 0.08 + 0.7), (-w * 0.08, d * 0.08 + 0.7)], 0.9),
        RAIL([(-w * 0.31, d * 0.08 - 0.7), (-w * 0.08, d * 0.08 - 0.7)], 0.9),
        # трубчатая горка от большой башни
        ('equip', dict(poly=_box(1.1, 5.4, -w * 0.33 - 3.6, d * 0.10 - 2.4), h=3.6)),
        # скалодром у малой башни
        ('equip', dict(poly=_box(3.2, 1.2, -w * 0.06 + 2.6, d * 0.06 - 1.6), h=3.0)),
        # качели: две А-рамы и перекладина
        ('column', dict(pts=[(w * 0.05, d * 0.22, 0.16), (w * 0.15, d * 0.22, 0.16)],
                        h=2.6)),
        ('equip', dict(poly=_box(w * 0.12, 0.26, w * 0.10, d * 0.22), h=2.7)),
        # круглый павильон на колоннах
        CAN(pav, 3.6, 4.0, 'flat', cols_ring(pav, 8, 0.26, 0.5), over=0.8),
        RAIL([(p[0], p[1]) for p in cols_ring(pav, 9, 0.1, 0.6)][4:], 1.5),
        # домик с горкой
        V(house, 2.4, 4.0, 'gable', over=0.5),
        ('equip', dict(poly=_box(0.9, 3.0, w * 0.44 - 2.6, -d * 0.24 - 1.6), h=1.6)),
