...
def hotel(w, d):
    """Двухэтажный корпус простой прямоугольной конфигурации (правка клиента):
    плоские разноуровневые кровли с парапетами, сплошное остекление обоих
    этажей, галерея-балкон на колоннах во всю длину главного фасада,
    входной блок со стеклянным козырьком. Поперечное крыло убрано."""
    bar = _box(w * 0.92, d * 0.52, 0.0, d * 0.10)            # единый прямоугольник
    core = _box(w * 0.34, d * 0.30, -w * 0.04, d * 0.10)     # верхний объём
    fy = d * 0.10 - d * 0.26                                 # линия главного фасада
    gal = _box(w * 0.88, 3.6, 0.0, fy - 1.8)
    cols = cols_line(-w * 0.43, fy - 3.4, w * 0.43, fy - 3.4, 11, 0.30)
    return [
        plinth(bar, 0.5),
        V(bar, 7.6, 8.1, 'flat', over=0.9),
        V(core, 10.4, 10.9, 'flat', over=0.7),               # третий уровень
        CAN(gal, 3.9, 4.15, 'flat', cols, over=0.35),
        RAIL([(-w * 0.44, fy - 3.5), (w * 0.44, fy - 3.5)], 1.05),
        win_band(-w * 0.44, w * 0.44, fy + 0.12, z0=0.7, h=2.9),
        win_band(-w * 0.44, w * 0.44, fy + 0.12, z0=4.4, h=2.6),
        win_band(-w * 0.44, w * 0.44, d * 0.36, z0=0.7, h=2.6),   # задний фасад
        door(0.0, fy - 0.1, 2.6),
        CAN(_box(w * 0.22, 4.4, 0.0, fy - 5.6), 4.4, 4.6, 'flat',
            cols_line(-w * 0.09, fy - 7.6, w * 0.09, fy - 7.6, 3, 0.26), over=0.4),
        ('deck', dict(poly=_box(w * 0.88, d * 0.14, 0.0, fy - 3.9))),
        steps(w * 0.18, fy - 7.8),
        ENT(0.0, fy - 8.0),
    ]
