...
def fence_wall(ring, gaps=(), h=2.2, t=0.30):
    """Забор единой стеной по периметру с разрывами в местах въездов.
    gaps — список (точка, ширина) в плане."""
    from shapely.geometry import LineString, Point as _P
    line = LineString(ring)
    wall = line.buffer(t / 2.0, cap_style=2, join_style=2)
    for pt, wgap in gaps:
        wall = wall.difference(_P(pt).buffer(wgap / 2.0))
    return wall
