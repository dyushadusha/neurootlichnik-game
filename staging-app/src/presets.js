...
/* Состояние объекта. Влияет не на картинку, а на состав работ:
   в новостройке нечего демонтировать, а в квартире с черновой
   отделкой стяжка и штукатурка уже частично сделаны застройщиком.
   Без этого вопроса смета врёт на сотни тысяч. */
const OBJECT_STATES = [
  {
    id: 'secondary',
    label: 'Вторичка',
    hint: 'Есть старая отделка — нужен демонтаж',
    factors: { demo: 1, screed: 1, plaster: 1 }
  },
  {
    id: 'new_rough',
    label: 'Новостройка, черновая',
    hint: 'Стяжка и штукатурка от застройщика',
    factors: { demo: 0, screed: 0.4, plaster: 0.5 }
  },
  {
    id: 'new_bare',
    label: 'Новостройка, голые стены',
    hint: 'Плита и блоки — черновые работы целиком наши',
    factors: { demo: 0, screed: 1, plaster: 1 }
  }
];

/* Три вопроса, которые отсекают пустой выезд. Замер стоит компании
   12–15 тыс ₽, поэтому ехать к тому, кто "просто узнавал цену", —
   прямой убыток. */
const QUALIFY_QUESTIONS = [
  { id: 'timing', label: 'Когда планируете начать?', bad: 'Ответ «когда-нибудь» — выезд подождёт' },
  { id: 'decision', label: 'Кто принимает решение по ремонту?', bad: 'Решает не он — нужен второй участник на замере' },
  { id: 'budget', label: 'Бюджет в озвученную вилку укладывается?', bad: 'Вилка не устроила — замер не спасёт' }
];

window.NS_PRESETS = { ROOM_TYPES, STYLE_PRESETS, OUTPUT_MODES, OBJECT_STATES, QUALIFY_QUESTIONS };
